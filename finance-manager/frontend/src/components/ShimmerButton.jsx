...
const ShimmerButton = ({
  ...
  const variants = {
    ...
    orange: {
      bg: "linear-gradient(to right, #F97316, #EA580C)",
      shimmerColor: "#FB923C",
    ...
    gray: {
      bg: "linear-gradient(to right, #525252, #262626)",
      shimmerColor: "#737373",
  ...
  const sharedClasses = `group relative z-0 text-white flex cursor-pointer items-center justify-center font-medium gap-2 overflow-hidden whitespace-nowrap border ${
    currentVariant.borderColor
  } ${currentSize} [background:var(--bg)] [border-radius:var(--radius)] transform-gpu transition-transform duration-300 ease-in-out active:translate-y-px ${
    disabled ? "opacity-50 cursor-not-allowed" : ""
  } ${className}`;

  const shimmerAndEffects = (
    <>
      {/* Shimmer Effect */}
      {!disabled && (
